...
class LidarThread(threading.Thread):
    def run(self):
        ...
        lidar_exe = subprocess.Popen(['ultra_simple.exe', 'COM3'], stdout=subprocess.PIPE)
...
class LidarProcessThread(threading.Thread):
    def run(self):
        ...
        while 1:
            ...
            quality = int(lidar_data.split()[2])
            ...
            lidar_field_raw[1, bin_num] = quality
...
class LidarPlotThread(threading.Thread):
    def run(self):
        ...
        while 1:
            ...
            plt.pause(0.01)
            if (count == 20):
                np.savetxt('D:/ADP/camera_testing/ZED/images/data.csv', lidar_field, delimiter = ',')
                count = 0
                print(lidar_field)
            count += 1
