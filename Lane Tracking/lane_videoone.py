...
class Lane:
  ...
  def compute_steering_angle( self, left_curvem , right_curvem):
    ...
    camera_mid_offset_percent = 0.02 # 0.0 means car pointing to center, -: car is centered to left, + means car pointing to right
  ...
  def stabilize_steering_angle(self, curr_steering_angle, new_steering_angle, num_of_lane_lines, max_angle_deviation_two_lines=10, max_angle_deviation_one_lane=1):
  ...
  def steer_angle(self, left_curvem,right_curvem ):
    ...
    curr_steering_angle = self.stabilize_steering_angle(self.curr_steering_angle, new_steering_angle, 2)
  ...
  def calculate_car_position(self, print_to_terminal=False):
    ...
    self.curr_steering_angle = -1*((self.left_curvem+self.right_curvem)/2)/np.absolute((self.left_curvem+self.right_curvem)/2) * curr_steering_angle       
  ...
  def display_curvature_offset(self, frame=None, plot=False):
    ...
    ptStart = (int(self.width/2), int(self.height))
    PT1=int(self.width/2-(self.height/4)*math.sin(self.curr_steering_angle*0.0174))
    PT2=self.height-int((self.height/4)*math.cos(self.curr_steering_angle*0.0174))
    ptEnd = (PT1, PT2)
    ...
    PT3=int(PT1-(self.height/7)*math.sin(2*self.curr_steering_angle*0.0174))
    PT4=PT2-int((self.height/7)*math.cos(2*self.curr_steering_angle*0.0174))
    ptStart = (PT1, PT2)
    ptEnd = (PT3, PT4)
    point_color = (0, 0, 225) # BGR
    thickness = 4
    ...
    PT5=int(PT3-(self.height/10)*math.sin(3*self.curr_steering_angle*0.0174))
    PT6=PT4-int((self.height/10)*math.cos(3*self.curr_steering_angle*0.0174))
    ptStart = (PT3, PT4)
    ptEnd = (PT5, PT6)
    point_color = (0, 0, 225) # BGR
    thickness = 4
    ...
    PT7=int(PT5-(self.height/14)*math.sin(4*self.curr_steering_angle*0.0174))
    PT8=PT6-int((self.height/14)*math.cos(4*self.curr_steering_angle*0.0174))
    ptStart = (PT5, PT6)
    ptEnd = (PT7, PT8)
    point_color = (0, 0, 225) # BGR
    thickness = 4
    ...
    ptStart = (int(self.width/2)-20, int(self.height/2)-20)
    ptEnd = (int(self.width/2)+20, int(self.height/2)+20)
    ...
    for point in points_list:
      cv2.circle(image_copy, point, point_size, point_color, thickness)
    point_size = 5
    point_color = (0, 0, 255) # BGR
    thickness = 4 # can be 0 、4、8
    points_list = [(PT3, PT4)]
    ...
    contours = np.array([[int(self.width/2)+int(self.curr_steering_angle)* 35 ,int(self.height/2)] , [ int(self.width/2) , int(self.height)] , [int(self.width/2)-int(self.curr_steering_angle)*35 , int(self.height/2)] ] )
    img = np.zeros((int(self.height), int(self.width) , 3), np.uint8) #make a grey frame
    cv2.fillPoly(img, pts = [contours], color =(0,255,0))
    result = cv2.addWeighted(image_copy, 1, img, 0.3, 0)
    image_copy=result 
    if plot==True:       
  ...
  def plot_roi(self, image):
    ...
    triangle = np.array([[(0, height), (width, height), (width-int(width/2), int(height/2))]])
  ...
  def overlay_lane_lines(self, plot=False):
    ...
    imgofpath = np.zeros((self.height, self.width, 3), np.uint8) #make a grey frame                             
    

    PT1=int(self.width/2-(self.height/7)*math.sin(self.curr_steering_angle*0.0174))
    PT2=self.height-int((self.height/7)*math.cos(self.curr_steering_angle*0.0174))
    PT3=int(PT1-(self.height/7)*math.sin(2*self.curr_steering_angle*0.0174))
    PT4=PT2-int((self.height/7)*math.cos(2*self.curr_steering_angle*0.0174))
    PT5=int(PT3-(self.height/7)*math.sin(3*self.curr_steering_angle*0.0174)) 
    PT6=PT4-int((self.height/7)*math.cos(3*self.curr_steering_angle*0.0174))
   
    contours = np.array([[int(self.width/2)+int(self.curr_steering_angle)* 50 ,int(self.height/2)] , [ int(self.width/2) , int(self.height)] , [int(self.width/2)-int(self.curr_steering_angle)*50 , int(self.height/2)] ] )

    cv2.fillPoly(imgofpath, pts = [contours], color =(225,225,225))

    # Combine the result with the original image
    result = cv2.addWeighted(self.orig_frame, 1, newwarp, 0.3, 0)
    result = cv2.addWeighted(result, 1, imgofpath, 0.3, 0)
    if plot==True:
